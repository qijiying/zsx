...
/*				itemF.getFileList().forEach(items->{
				...
				for (int i = 0; i < itemF.getFileList().size(); i++) {
					...
					if(groupId.equals(str)&&flag==ConstantClass.ONE_FLAG){
						...
						if(i==0&&flag==ConstantClass.TWO_FLAG&&itemF.getFileList().get(1).getFileStyle()==2)
							itemF.getFileList().get(1).setFileName(ServerBase.getServer()+itemF.getFilePath()+itemF.getFileList().get(1).getFileName());
						   
						if(flag==ConstantClass.TWO_FLAG&&itemF.getFileList().get(i).getFileStyle()!=2){
							itemF.getFileList().remove(i);
							//list操作remove后长度减一,如果i=0，则减一会出异常，需额外处理
							if(i!=0)i--;
						}else itemF.getFileList().get(i).setFileName(ServerBase.getServer()+itemF.getFilePath()+itemF.getFileList().get(i).getFileName());
